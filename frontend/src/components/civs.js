...
class Civs extends Component {
  ...
  componentDidMount() {
    fetch('/api/v1/civ_win_rates/'.concat(this.props.location.search))
      .then((res) => res.json())
      .then((data) => {
        data.civs_list = data.civs_list.filter((civ) => civ.total > 0);
  ...
  render() {
    ...
    if (this.state.civilizations.hasOwnProperty('civs_list')) {
      return (
        <div>
          <Input defaultmirror ref={this.input} callback={this.handleSubmit} parent_query={this.query_params} />
          <h3>
            {' '}
            Total Games in Query:
            {this.state.civilizations.total}
          </h3>
