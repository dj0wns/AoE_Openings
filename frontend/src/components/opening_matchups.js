...
class OpeningWins extends Component {
  ...
  componentDidMount() {
    fetch('http://127.0.0.1:8000/api/v1/opening_matchups/'.concat(this.props.location.search))
     .then(res => res.json())
     .then((data) => {
  ...
  handleSubmit(data) {
    ...
    fetch('http://127.0.0.1:8000/api/v1/opening_matchups/'.concat(query_string))
      .then(res => res.json())
      .then((data) => {
        data.openings_list = data.openings_list.filter(function(civ) {return civ.total > 0})
        // Filter out openings with 0 matches
